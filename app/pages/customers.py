...
from app.components.charts import plot_donut_chart, plot_bar_chart
...
def render():
    ...
    merged_cust = df_cust.merge(risk_df[["customer_id", "risk_probability", "risk_band"]], on="customer_id", how="left")
    ...
    with col2:
        ...
        render_data_table(high_risk[["customer_id", "customer_segment", "recency_days", "frequency", "monetary_total", "risk_probability", "risk_band"]].head(10))
    ...
    if selected_cid:
        ...
        with c1:
            render_kpi_card("Risk Score", f"{c_row['risk_probability']:.1%}", f"Band: {c_row['risk_band']}", "#EF4444" if c_row['risk_band'] == "High" else "#10B981")
        with c2:
            render_kpi_card("Inactivity Recency", f"{int(c_row['recency_days'])} Days", f"Segment: {c_row['customer_segment']}")
        with c3:
            render_kpi_card("Lifetime Spend", f"${c_row['monetary_total']:,.2f}", f"Orders: {int(c_row['frequency'])}")
